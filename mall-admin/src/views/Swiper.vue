<template>
    <div>
        <el-card class="swiper-container">
            <!-- 内部 slot(插槽) 区域 -->
            <!-- 定制卡片内容 slot -->
            <!-- 不是id选择器 ，名字叫header 具名slot -->
            <template #header>
                <div class="header">
                    <el-button type="primary" 
                        @click="addSwiper"
                        :icon="Plus">增加</el-button>
                    <el-popconfirm
                        title="确定要删除吗？"
        ...
        >
            <el-form-item label="图片" prop="url">
                ...
                >
                    <img :src="state.swiperFrom.url"
                        class="avatar"
                        v-if="state.swiperFrom.url"
            ...
            <el-form-item label="跳转链接" prop="link">
                <el-input type="text" v-model="state.swiperFrom.link"></el-input>
            </el-form-item>
            <el-form-item label="排序值" prop="sort">
                <el-input type="number" v-model="state.swiperFrom.sort"></el-input>
            </el-form-item>
        </el-form>
        <template #footer>
...
<script setup>
    import { reactive, ref, onMounted, TransitionGroup } from 'vue';
    import { getSwiper, addSwiperData } from '../service/swiper'
    ...
    const submitForm = () => {
        formRef.value.validate(async(valid) => {
            if(valid) {
                await addSwiperData({
                    carouselUrl: state.swiperFrom.url[0],
                    redirectUrl: state.swiperFrom.link,
                    carouselRank: state.swiperFrom.sort
                })
    ...
    const handleUrlSuccess = (data) => {
        ...
        state.swiperFrom.url = data.data || ''
    }
